#!/bin/bash
# =============================================================================
# Stage 2 ZeRO-2 full fine-tuning: 2-node distributed training for LLaVA
# =============================================================================
# Usage:
#   1. Open two terminals and allocate one ws-ia GPU node in each:
#        Terminal A: salloc -p ws-ia -N1 -n12 --mem=64G -w ws-l4-XXX
#        Terminal B: salloc -p ws-ia -N1 -n12 --mem=64G -w ws-l4-YYY
#   2. Update master_node and worker_node below with the allocated hostnames
#   3. Run on BOTH terminals:
#        eval "$(conda shell.bash hook)" && conda activate llava
#        bash scripts/run_stage2_zero2_full_ft.sh
# =============================================================================
...
export OUTPUT_DIR="$PROJECT_DIR/checkpoints/llava-stage2-zero2-full-ft"
...
setup_run_logging "stage2_zero2_full_ft" "Stage 2 ZeRO-2 Full FT (rank $NODE_RANK)" "$PER_DEVICE_BATCH" "$GRAD_ACCUM" "$WORLD_SIZE"
trap cleanup_run_logging EXIT

echo "=========================================="
echo "Stage 2 ZeRO-2 full fine-tuning"
echo "=========================================="
...
if [ "$TRAIN_EXIT" -ne 0 ]; then
    echo "=== Stage 2 ZeRO-2 full fine-tuning failed on $CURRENT_HOST (rank $NODE_RANK, exit code: $TRAIN_EXIT) ==="
    exit "$TRAIN_EXIT"
fi

echo "=== Stage 2 ZeRO-2 full fine-tuning complete on $CURRENT_HOST ==="
