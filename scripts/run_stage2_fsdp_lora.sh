#!/bin/bash
# =============================================================================
# Stage 2 PyTorch FSDP LoRA fine-tuning for LLaVA
# =============================================================================
# PyTorch-native equivalent of DeepSpeed ZeRO-3: full sharding of parameters,
# gradients, and optimizer states across the data-parallel ranks.
#
# Usage:
#   1. Allocate one GPU node in two terminals (ws-ia partition).
#   2. Set MASTER_NODE / WORKER_NODE to the two hostnames.
#   3. Run this script on both nodes.
# =============================================================================
...
export OUTPUT_DIR="${OUTPUT_DIR:-$PROJECT_DIR/checkpoints/llava-stage2-fsdp-lora}"
export LOG_DIR="${LOG_DIR:-$PROJECT_DIR/logs}"
export FSDP_CONFIG="${FSDP_CONFIG:-$PROJECT_DIR/configs/fsdp.json}"
# HF Trainer --fsdp value. "full_shard auto_wrap" = ZeRO-3-equivalent.
# Other valid combos: "shard_grad_op auto_wrap" (ZeRO-2-eq), "hybrid_shard auto_wrap".
...
setup_run_logging "stage2_fsdp_lora" "Stage 2 FSDP LoRA (rank $NODE_RANK)" "$PER_DEVICE_BATCH" "$GRAD_ACCUM" "$WORLD_SIZE"
trap cleanup_run_logging EXIT

echo "=========================================="
echo "Stage 2 PyTorch FSDP LoRA fine-tuning"
echo "=========================================="
...
if [ "$TRAIN_EXIT" -ne 0 ]; then
    echo "=== Stage 2 FSDP LoRA failed on $CURRENT_HOST (rank $NODE_RANK, exit code: $TRAIN_EXIT) ==="
    exit "$TRAIN_EXIT"
fi

echo "=== Stage 2 FSDP LoRA complete on $CURRENT_HOST ==="
