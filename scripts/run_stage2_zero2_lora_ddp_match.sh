#!/bin/bash
# =============================================================================
# Stage 2 ZeRO-2 LoRA fine-tuning with DDP-matched hyperparameters
# =============================================================================
...
export OUTPUT_DIR="${OUTPUT_DIR:-$PROJECT_DIR/checkpoints/llava-stage2-zero2-lora-ddp-match}"
...
setup_run_logging "stage2_zero2_lora_ddp_match" "Stage 2 ZeRO-2 LoRA DDP-match (rank $NODE_RANK)" "$PER_DEVICE_BATCH" "$GRAD_ACCUM" "$WORLD_SIZE"
trap cleanup_run_logging EXIT

echo "=========================================="
echo "Stage 2 ZeRO-2 LoRA fine-tuning"
echo "DDP-matched hyperparameters"
echo "=========================================="
...
if [ "$TRAIN_EXIT" -ne 0 ]; then
    echo "=== Stage 2 ZeRO-2 LoRA DDP-match failed on $CURRENT_HOST (rank $NODE_RANK, exit code: $TRAIN_EXIT) ==="
    exit "$TRAIN_EXIT"
fi

echo "=== Stage 2 ZeRO-2 LoRA DDP-match complete on $CURRENT_HOST ==="
